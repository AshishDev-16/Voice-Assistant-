 ...
 */
export const generateKnowledgeDraft = async (
  businessName: string,
  businessType: string,
  agentGoal: string
): Promise<string> => {
  try {
    ...
    if (!apiKey || apiKey.includes('your-')) {
      logger.warn('Valid GEMINI_API_KEY is missing. Using high-quality mock draft.');
    ...
    logger.info('Providing high-quality mock draft fallback.');
    return generateMockDraft(businessName, businessType, agentGoal);
  }
};
 ...
 */
export const generateCallIntelligence = async (
  ...
  try {
    ...
    logger.error('Error in generateCallIntelligence:', error);
    return {
 ...
 */
const generateMockDraft = (name: string, type: string, goal: string) => {
